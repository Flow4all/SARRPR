...
if __name__ == "__main__":
    ...
    for i in range(len(frases)):    #recorremos las frases
        ...
        if not frases[i]=="":
            ...
            for j in range(len(palabras)-2):    #recorremos las palabras de la frase i 
                palabra = palabras[j]
                palabra_sucesora = palabras[j+1]
                palabra_sucesora2 = palabras[j+2]
                valor = d.get(palabra+" "+palabra_sucesora,[0,{}])
                dic_sucesor = valor[1]
                valor_sucesor = dic_sucesor.get(palabra+" "+palabra_sucesora+" "+palabra_sucesora2,0)
                dic_sucesor[palabra+" "+palabra_sucesora+" "+palabra_sucesora2] = valor_sucesor+1    #insertar apariciones j y j+1 en diccionario auxiliar 
                d[palabra+" "+palabra_sucesora] = [valor[0]+1,dic_sucesor]    #insertamos el valor de la palabra j (nºapariciones,diccionario auxiliar)
    for i in d.keys():    #recorremos el diccionario d para insertarlo ordenado en el diccionario f
        dic_sucesor = d[i][1]
        ...
        for j in freq:    #cada par clave/valor ordenados por valor lo insertamos en la posting list de la clave i
            palabra_sucesora = (str(j[0]).split(" "))[2]
